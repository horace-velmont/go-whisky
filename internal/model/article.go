...
func (m ArticleModel) Update(userID int64, id int64, form forms.CreateArticleForm) (err error) {
	...
	operation, err := db.GetDB2().Exec("UPDATE public.article SET title=$2, content=$3 WHERE id=$1", id, form.Title, form.Content)
	if err != nil {
		return err
	}
...
func (m ArticleModel) Delete(userID, id int64) (err error) {
	...
	operation, err := db.GetDB2().Exec("DELETE FROM public.article WHERE id=$1", id)
	if err != nil {
		return err
	}
