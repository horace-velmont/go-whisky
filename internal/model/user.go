...
func (m UserModel) Login(form forms.LoginForm) (user User, token Token, err error) {
	...
	err = db.GetDB2().SelectOne(&user, "SELECT id, email, password, name, updated_at, created_at FROM public.user WHERE email=LOWER($1) LIMIT 1", form.Email)
...
func (m UserModel) One(userID int64) (user User, err error) {
	err = db.GetDB2().SelectOne(&user, "SELECT id, email, name FROM public.user WHERE id=$1 LIMIT 1", userID)
	return user, err
}
